...
function UserLogin($email, $password) {
	$query = "SELECT id, first_name, last_name, email, gender, nationality, permission FROM register WHERE email = :email AND password = :password";
	$params = [
		':email' => $email,
...
function UserRegister($email, $password, $fname, $lname, $gender, $nationality) {
	$query = "SELECT id FROM register email = :email";
	$params = [ ':email' => $email ];

	require_once DATABASE_CONTROLLER;
	$record = getRecord($query, $params);
	if(empty($record)) {
		$query = "INSERT INTO register (first_name, last_name, email, password, gender, nationality) VALUES (:first_name, :last_name, :email, :password, :gender, :nationality)";
		$params = [
			':first_name' => $fname,
			':last_name' => $lname,
			':email' => $email,
